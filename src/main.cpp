...
//	double cone_half_angle = 0.5*M_PI/180.0;
    ...
    // Cone inner_geometry(origin, direction, cone_half_angle/10, big_scale);

    // Setting components of B-fields ==================================================================================
    double pitch_angle_deg = 89.;
	// HelicalConicalBField jetbfield(2.0, 1, pitch_angle_deg*M_PI/180., true, 0.0, &geometry);
    double Psi_tot = 1E+33;
    double sigma_m = 5;
    double Omega = 1E-6;
    BeskinMHDBField jetbfield(Psi_tot, sigma_m, Omega, los_angle);
    ...
    PowerLaw particles(s, gamma_min, "pairs", false, ds);
    // Value at r = 1 pc
    // For GJ K_1 = 1E17;
    // For B2 K_1 = 1E12;
    // For j2 K_1 = 1E22;
	...
	double n = 1.5;
    // Посмотреть v_phi = 0.1 c и Omega -> -Omega

    // Setting V-field =================================================================================================
    VField* vfield;
    BeskinVfield vfield_(&geometry, &jetbfield, Psi_tot, sigma_m, Omega, los_angle);
    vfield_.set_v_phi_factor(0.);
    vfield = &vfield_;
    ...
    // BKNField nfield(K_1, n, &particles, false, &geometry, nullptr, vfield);
    // nfield.set_heating_profile(0.9, 0.037, 0.01);
	// Equipartition particles density
    // EquipartitionBKNfield power_law_nfield_spine(&particles, vbfields, &geometry, nullptr, vfield);
    // B2Nfield nfield(K_1, Psi_tot, sigma_m, Omega, los_angle, vbfields, &particles, true, &geometry, nullptr, vfield);
    GJNfield nfield(K_1, Psi_tot, sigma_m, Omega, los_angle, &particles, true, &geometry, nullptr, vfield);
    nfield.set_heating_profile(0.6, 0.3, 0.00);
...
//        string polarization = "I";
    ...
    std::remove("Jetmodel_parameters.txt");
    std::ofstream fout("Jetmodel_parameters.txt");
    fout << redshift << std::endl << pitch_angle_deg << std::endl << number_of_pixels_along
    << std::endl << number_of_pixels_across << std::endl << pixel_size_mas_start << std::endl 
    << pixel_size_mas_stop << std::endl
    << pow(10.0, lg_pixel_size_start) / pc / pc_in_mas << std::endl 
    << pow(10.0, lg_pixel_size_stop) / pc / pc_in_mas << std::endl;
    fout.close();
