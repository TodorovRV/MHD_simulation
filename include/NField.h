...
class GJNfield: public NField {
    public:
        GJNfield(double n_0, double Psi_tot, double sigma_m, double Omega, double theta,
        ParticlesDistribution* particles, bool in_plasma_frame, Geometry* geometry_out, Geometry* geometry_in = nullptr, VField* vfield = nullptr);
        double _nf(const Vector3d &point) const override;
        void set_heating_profile(double r_mean, double r_width, double background_fraction = 0.0);
    private:
        double n_0_;
...
class B2Nfield: public NField {
    public:
        B2Nfield(double n_0, double Psi_tot, double sigma_m, double Omega, double theta, std::vector<VectorBField*> vbfields,
        ParticlesDistribution* particles, bool in_plasma_frame, Geometry* geometry_out, Geometry* geometry_in = nullptr, VField* vfield = nullptr);
        double _nf(const Vector3d &point) const override;
        void set_heating_profile(double r_mean, double r_width, double background_fraction = 0.0);
    private:
...
class j2Nfield: public NField {
    public:
        j2Nfield(double n_0, double Psi_tot, double sigma_m, double Omega, double theta, std::vector<VectorBField*> vbfields,
        ParticlesDistribution* particles, bool in_plasma_frame, Geometry* geometry_out, Geometry* geometry_in = nullptr, VField* vfield = nullptr);
        double _nf(const Vector3d &point) const override;
    private:
        double n_0_;
